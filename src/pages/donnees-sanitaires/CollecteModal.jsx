...
export default function CollecteModal({ isOpen, onClose, onSuccess }) {
  ...
  const [formData, setFormData] = useState({
    ...
    notes: '',
    est_anonyme: true,          // 🆕 Par défaut anonyme
    nom_patient: '',            // 🆕
    prenom_patient: '',         // 🆕
    telephone_patient: '',      // 🆕
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setMessage({ type: 'success', text: '✅ Donnée collectée avec succès !' });
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ backgroundColor: 'rgba(0,0,0,0.5)' }}>
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          {/* ✅ NOUVEAU : TOGGLE ANONYMAT */}
          <div className="mb-4">
            <label className="flex items-center gap-2 cursor-pointer">
              <input
                type="checkbox"
                checked={formData.est_anonyme}
                onChange={(e) => {
                  const isAnon = e.target.checked;
                  setFormData(prev => ({
                    ...prev,
                    est_anonyme: isAnon,
                    // Si anonyme, vider nom/prénom
                    ...(isAnon ? { nom_patient: '', prenom_patient: '', telephone_patient: '' } : {})
                  }));
                }}
              ...
              <span className="text-sm font-medium text-gray-700">
                🔒 Données anonymes (recommandé pour statistiques)
              </span>
            </label>
          </div>

          {/* ✅ SI NON ANONYME : Afficher champs identité */}
          {!formData.est_anonyme && (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 p-4 rounded-xl" style={{ backgroundColor: '#FFF0E6', border: '1px solid #FF6B35' }}>
              <div className="md:col-span-3 flex items-center gap-2 text-sm font-medium" style={{ color: '#FF6B35' }}>
                ⚠️ Attention : Ces informations seront visibles par tous les utilisateurs autorisés
              </div>
              
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">
                  Nom <span style={{ color: '#DC143C' }}>*</span>
                </label>
                <input
                  type="text"
                  name="nom_patient"
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">
                  Prénom <span style={{ color: '#DC143C' }}>*</span>
                </label>
                <input
                  type="text"
                  name="prenom_patient"
              ...
              <div>
                ...
                <input
                  ...
                  onChange={handleChange}
                  className="w-full px-4 py-2.5 border border-gray-200 rounded-xl focus:outline-none"
                  placeholder="Ex: +236 XX XX XX XX"
                />
              </div>
            </div>
          )}
